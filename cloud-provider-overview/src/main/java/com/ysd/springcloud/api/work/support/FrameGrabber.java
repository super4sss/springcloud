...
public class FrameGrabber {
	...
	public BufferedImage grabBufferImage(String url) throws IOException {
		...
		ByteBuffer buf = grabVideoFrame(url, AV_PIX_FMT_BGR24);
		return ImageConverter.BGR2BufferedImage(buf, width, height);
	}
	 ...
	 * 获取第一帧视频位置
	 */
	private int findVideoStreamIndex(AVFormatContext pFormatCtx) {
	 ...
	 * 查找并尝试打开解码器
	 */
	private AVCodecContext findAndOpenCodec(AVCodecContext pCodecCtx) {
		// Find the decoder for the video stream
	 ...
	 * BGR图像帧转字节缓冲区（BGR结构）
	 */
	private ByteBuffer saveFrame(AVFrame pFrame, int width, int height) {
		int size = width * height * 3;
		return pFrame.data(0).position(0).limit(size).asBuffer();
	}
